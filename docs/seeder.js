...
class AbilityScores {
    ...
    diff(other) {
        let ret = Object.getOwnPropertyNames(AbilityKind).map((k) => [k, other.value(k) - this.value(k)]);
        return ret;
    }
    clone() {
...
class Character {
    constructor(name = Character.nextName(), abilityScores = new abilityScore_1.AbilityScores(), race = new race_1.Race(), characterClass, background = new background_1.Background(background_1.BackgroundKind.Criminal), alignment = Alignment.ChaoticGood(), experience = 0, height = new Height(), weight = 160, eyeColor = 'grey', inspiration = 0, armor = null, shield = null, skills = new skills_1.Skills(), weapons = [new Weapon()], wealth = new Wealth(), languages = [NormalLanguage.Common], damage = 0, tempHp = 0, magicItems = [], expendables = []) {
        this.name = name;
    ...
    modifiedAbilityScores() {
        return this.abilityScores.map(s => {
            ...
            return new abilityScore_1.AbilityScore(bonus + s.value, s.kind);
        });
    }
    get passiveWisdom() {
    ...
    get initiative() {
        return this.modifiedAbilityScores().find(s => s.kind === abilityScore_1.AbilityKind.Dexterity).modifier;
    }
    armorClass() {
        let dex = this.modifiedAbilityScores().find(a => a.kind === abilityScore_1.AbilityKind.Dexterity).modifier;
        let bonus = 0;
        if (this.armor) {
    ...
    abilityScoreModNeeded() {
        let currentBonusPoints = data_1.Data.getAbilityScoreBonusCountFor(this.characterClass.name, this.level);
...
class Class {
    constructor(name, _level, classDetails, bonusAbilityScores = exports.DEFAULT_BONUS_ABILITY_SCORES, selectedSkills = []) {
        this.name = name;
        this._level = _level;
        ...
        switch (name) {
            case ClassKind.Barbarian:
                this.barbarianCtor(classDetails);
                break;
            case ClassKind.Bard:
                this.bardCtor(classDetails);
                break;
            case ClassKind.Cleric:
                this.clericCtor(classDetails);
                break;
            case ClassKind.Druid:
                this.druidCtor(classDetails);
            case ClassKind.Fighter:
                this.fighterCtor(classDetails);
                break;
            case ClassKind.Monk:
                this.monkCtor(classDetails);
                break;
            case ClassKind.Paladin:
                this.paladinCtor(classDetails);
                break;
            case ClassKind.Ranger:
                this.rangerCtor(classDetails);
                break;
            case ClassKind.Rogue:
                this.rogueCtor(classDetails);
                break;
            case ClassKind.Sorcerer:
                this.sorcererCtor(classDetails);
                break;
            case ClassKind.Warlock:
                this.warlockCtor(classDetails);
                break;
            case ClassKind.Wizard:
                this.wizardCtor(classDetails);
                break;
        }
    }
    get level() {
    ...
    bardCtor(details = new classDetails_1.BardDetails(this._level, null, [])) {
        ...
        if (this.classDetails && this.classDetails.archetype === classDetails_1.BardCollege.Valor && this._level > 2) {
    ...
    clericCtor(details) {
        ...
        this.numberOfSkills = 2;
        if (this.classDetails.archetype === classDetails_1.ClericDomain.Knowledge
            || this.classDetails.archetype === classDetails_1.ClericDomain.Nature) {
            this.numberOfSkills += 2;
        }
        this.availableSkills = [
    ...
    get isCaster() {
        return this.name == ClassKind.Bard
            ...
            || (this.name == ClassKind.Fighter
                && this.classDetails.archetype == classDetails_1.CombatArchetype.EldrichKnight)
            || (this.name == ClassKind.Rogue
                && this.classDetails.archetype == classDetails_1.RoguishArchetype.ArcaneTrickster);
    }
    bonusAbilityCount() {
    ...
    unselectedAvailableSkills() {
        ...
        if (this.name === ClassKind.Cleric) {
            if (this.classDetails.archetype === classDetails_1.ClericDomain.Knowledge) {
                return [skills_1.SkillKind.Arcana, skills_1.SkillKind.History, skills_1.SkillKind.Nature, skills_1.SkillKind.Religion].filter(s => this.selectedSkills.includes(s));
            }
            if (this.classDetails.archetype === classDetails_1.ClericDomain.Nature) {
                return [skills_1.SkillKind.AnimalHandling, skills_1.SkillKind.Nature, skills_1.SkillKind.Survival].filter(s => this.selectedSkills.includes(s));
            }
        }
        return [];
    }
    static fromJson(json) {
        let ret = new Class(null, json._level, null, json.bonusAbilityScores, json.selectedSkills || []);
        ret.name = json.name;
        switch (json.name) {
            case ClassKind.Barbarian:
...
const data_1 = __webpack_require__(/*! ../services/data */ "./ts/services/data.ts");
class _ClassDetails {
    constructor(level, features, ARCHETYPE_KEY, selectedFeatures, id) {
        this.level = level;
        this.features = features;
        this.ARCHETYPE_KEY = ARCHETYPE_KEY;
    ...
    getAllAvailableFeatures() {
        return this.features.reduce((acc, f) => {
    ...
    get archetype() {
        ...
        if (this.archetypeIdx) {
            return this.features[this.archetypeIdx].options[idx].name;
        }
        for (let i = 0; i < this.features.length; i++) {
            let feat = this.features[i];
            if (feat.name === this.ARCHETYPE_KEY) {
                this.archetypeIdx = i;
                return feat.options[idx].name;
            }
        }
        return null;
    }
}
exports._ClassDetails = _ClassDetails;
class BarbarianDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
    ...
    static fromJson(json) {
        return new BarbarianDetails(json.level, json.features, json.selectedFeatures, json.id);
    }
}
...
class BardDetails extends _ClassDetails {
    constructor(level, features, expertise = [], inspiration = 0, selectedFeatures, id) {
    ...
    static fromJson(json) {
        return new BardDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.expertise, json.inspiration, json.selectedFeatures, json.id);
    }
}
...
class ClericDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
        super(level, features, 'Divine Domain', selectedFeatures, id);
    }
    static fromJson(json) {
        return new ClericDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
...
class DruidDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
        super(level, features, 'Druid Circle', selectedFeatures, id);
    }
    static fromJson(json) {
        return new DruidDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
...
class FighterDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
    ...
    static fromJson(json) {
        return new FighterDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
...
var CombatArchetype;
(function (CombatArchetype) {
    CombatArchetype[CombatArchetype["BattleMaster"] = 0] = "BattleMaster";
    CombatArchetype[CombatArchetype["Champion"] = 1] = "Champion";
    CombatArchetype[CombatArchetype["EldrichKnight"] = 2] = "EldrichKnight";
})(CombatArchetype = exports.CombatArchetype || (exports.CombatArchetype = {}));
var CombatSuperiority;
(function (CombatSuperiority) {
    CombatSuperiority["CommandersStrike"] = "Commander's Strike";
    CombatSuperiority["DisarmingAttack"] = "Disarming Attack";
    CombatSuperiority["DistractingStrike"] = "Distracting Strike";
    CombatSuperiority["EvasiveFootwork"] = "Evasive Footwork";
    CombatSuperiority["FeintingAttack"] = "Feinting Attack";
    CombatSuperiority["GoadingAttack"] = "Goading Attack";
    CombatSuperiority["LungingAttack"] = "Lunging Attack";
    CombatSuperiority["ManeuveringAttack"] = "Maneuvering Attack";
    CombatSuperiority["MenacingAttack"] = "Menacing Attack";
    CombatSuperiority["Parry"] = "Parry";
    CombatSuperiority["PrecisionAttack"] = "Precision Attack";
    CombatSuperiority["PushingAttack"] = "Pushing Attack";
    CombatSuperiority["Rally"] = "Rally";
    CombatSuperiority["Riposte"] = "Riposte";
    CombatSuperiority["SweepingAttack"] = "Sweeping Attack";
    CombatSuperiority["TripAttack"] = "Trip Attack";
})(CombatSuperiority = exports.CombatSuperiority || (exports.CombatSuperiority = {}));
class MonkDetails extends _ClassDetails {
    constructor(level, features, kiPoints = 0, selectedFeatures, id) {
    ...
    static fromJson(json) {
        return new MonkDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.kiPoints, json.selectedFeatures, json.id);
    }
}
exports.MonkDetails = MonkDetails;
var MonasticTradition;
(function (MonasticTradition) {
    MonasticTradition["Shadow"] = "Way of Shadow";
    MonasticTradition["OpenHand"] = "Way of the Open Hand";
    MonasticTradition["FourElements"] = "Way of the Four Elements";
    MonasticTradition["LongDeath"] = "Way of the Long Death";
    MonasticTradition["SunSoul"] = "Way of the Sun Soul";
})(MonasticTradition = exports.MonasticTradition || (exports.MonasticTradition = {}));
class PaladinDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
    ...
    static fromJson(json) {
        return new PaladinDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
...
(function (PaladinOath) {
    ...
    PaladinOath["Oathbreaker"] = "Oathbreaker";
})(PaladinOath = exports.PaladinOath || (exports.PaladinOath = {}));
class RangerDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
    ...
    static fromJson(json) {
        return new RangerDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
...
var RangerStyle;
(function (RangerStyle) {
    RangerStyle["Archery"] = "Archery";
    RangerStyle["Defense"] = "Defense";
    RangerStyle["Dueling"] = "Dueling";
    RangerStyle["TwoWeapon"] = "Two-Weapon Fighting";
})(RangerStyle = exports.RangerStyle || (exports.RangerStyle = {}));
var RangerArchetype;
(function (RangerArchetype) {
    RangerArchetype["Hunter"] = "Hunter";
    RangerArchetype["BeastMaster"] = "Beast Master";
})(RangerArchetype = exports.RangerArchetype || (exports.RangerArchetype = {}));
class RogueDetails extends _ClassDetails {
    constructor(level, features, expertise = [], bonusAbilityScore = [], selectedFeatures, id) {
        super(level, features, 'Roguish Archetype', selectedFeatures, id);
        this.expertise = expertise;
        this.bonusAbilityScore = bonusAbilityScore;
    }
    static fromJson(json) {
        return new RogueDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.expertise, json.bonusAbilityScore, json.selectedFeatures, json.id);
    }
}
exports.RogueDetails = RogueDetails;
class SorcererDetails extends _ClassDetails {
    constructor(level, features, sorcererPoints = 0, selectedFeatures, id) {
        super(level, features, 'Sorcerous Origin', selectedFeatures, id);
        this.sorcererPoints = sorcererPoints;
    }
    static fromJson(json) {
        return new SorcererDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.sorcererPoints, json.selectedFeatures, json.id);
    }
}
...
class WarlockDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
        super(level, features, 'Otherworldly Patron', selectedFeatures, id);
    }
    static fromJson(json) {
        return new WarlockDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
exports.WarlockDetails = WarlockDetails;
class EldritchIncantation {
    constructor(name, prerequisite, desc) {
        this.name = name;
        this.prerequisite = prerequisite;
        this.desc = desc;
    }
}
exports.EldritchIncantation = EldritchIncantation;
var EldrichIncantationName;
(function (EldrichIncantationName) {
    EldrichIncantationName["ArmorOfShadows"] = "Armor of Shadows";
    EldrichIncantationName["BeastSpeech"] = "Beast Speech";
    EldrichIncantationName["BeguilingInfluence"] = "Beguiling Influence";
    EldrichIncantationName["BookOfAncientSecrets"] = "Book of Ancient Secrets";
    EldrichIncantationName["DevilsSight"] = "Devil's Sight";
    EldrichIncantationName["EldritchSight"] = "Eldritch Sight";
    EldrichIncantationName["EldritchSpear"] = "Eldritch Spear";
    EldrichIncantationName["EyesOfTheRuneKeeper"] = "Eyes of the Rune Keeper";
    EldrichIncantationName["FiendishVigor"] = "Fiendish Vigor";
    EldrichIncantationName["GazeOfTwoMinds"] = "Gaze of Two Minds";
    EldrichIncantationName["MaskOfManyFaces"] = "Mask of Many Faces";
    EldrichIncantationName["MistyVisions"] = "Misty Visions";
    EldrichIncantationName["RepellingBlast"] = "Repelling Blast";
    EldrichIncantationName["ThiefOfFiveFates"] = "Thief of Five Fates";
    EldrichIncantationName["VoiceOfTheChainMaster"] = "Voice of the Chain Master";
})(EldrichIncantationName = exports.EldrichIncantationName || (exports.EldrichIncantationName = {}));
...
class WizardDetails extends _ClassDetails {
    constructor(level, features, selectedFeatures, id) {
        super(level, features, 'Arcane Tradition', selectedFeatures, id);
    }
    static fromJson(json) {
        return new WizardDetails(json.level, json.features.map(data_1.ClassFeature.fromJson), json.selectedFeatures, json.id);
    }
}
...
var RoguishArchetype;
(function (RoguishArchetype) {
    RoguishArchetype["Thief"] = "Thief";
    RoguishArchetype["Assassin"] = "Assassin";
    RoguishArchetype["ArcaneTrickster"] = "Arcane Trickster";
})(RoguishArchetype = exports.RoguishArchetype || (exports.RoguishArchetype = {}));
...
class Data {
    ...
    static getAbilityScoreBonusCountFor(cls, level) {
        let ret = 0;
        if (level > 3) {
            ret += 2;
        }
        ...
        if (cls === class_1.ClassKind.Rogue && level > 9) {
            ret += 2;
        }
        if (cls === class_1.ClassKind.Fighter) {
            if (level > 5) {
                ret += 2;
            }
            if (level > 13) {
...
class Database extends dexie_1.default {
    ...
    upgradeToTwo(t) {
        return __awaiter(this, void 0, void 0, function* () {
            for (let table in t.tables) {
                ...
                if (idx > -1) {
                    let classKind = formatClassKind(table.substring(0, idx));
                    let rows = yield t.table(table).toArray();
                    for (let row of rows) {
                        let spell = yield t.table('spells').where('name').equals(row.name).first();
                        if (spell) {
                            spell.classKinds.push(classKind);
                            yield t.table('spells').put(spell);
                        }
                        else {
                            delete row.id;
                            row.classKinds = [classKind];
                            yield t.table('spells').add(row);
                        }
                    }
                }
            }
    ...
    init() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let lastSeed = yield this.seeds.orderBy('id').reverse().first();
            if (!lastSeed) {
    ...
    saveCharacter(ch) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let dbCh = character_1.Character.fromJson(yield this.characters.get(ch.id));
                if (dbCh.level < ch.level) { // Level up!
                    for (let i = 0; i < ch.level - dbCh.level; i++) {
                        let newFeatures = yield this.getSingleLevelFeatures(ch.characterClass.name, dbCh.level + i + 1);
    ...
    spellsForClass(cls) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                let dbSpells = yield this.spells.where('classKinds').equals(cls).toArray();
    ...
    getClassDetails(cls, level) {
        return __awaiter(this, void 0, void 0, function* () {
            let features = yield this.getClassFeatures(cls, level);
            switch (cls) {
                case class_1.ClassKind.Barbarian:
                    return new classDetails_1.BarbarianDetails(level, features);
                case class_1.ClassKind.Bard:
                    return new classDetails_1.BardDetails(level, features);
                case class_1.ClassKind.Cleric:
                    return new classDetails_1.ClericDetails(level, features);
                case class_1.ClassKind.Druid:
                    return new classDetails_1.DruidDetails(level, features);
                case class_1.ClassKind.Fighter:
                    return new classDetails_1.FighterDetails(level, features);
                case class_1.ClassKind.Monk:
                    return new classDetails_1.MonkDetails(level, features);
                case class_1.ClassKind.Paladin:
                    return new classDetails_1.PaladinDetails(level, features);
                case class_1.ClassKind.Ranger:
                    return new classDetails_1.RangerDetails(level, features);
                case class_1.ClassKind.Rogue:
                    return new classDetails_1.RogueDetails(level, features);
                case class_1.ClassKind.Sorcerer:
                    return new classDetails_1.SorcererDetails(level, features);
                case class_1.ClassKind.Warlock:
                    return new classDetails_1.WarlockDetails(level, features);
                case class_1.ClassKind.Wizard:
                    return new classDetails_1.WizardDetails(level, features);
            }
        });
    }
    getClassFeatures(cls, level, optionId = undefined) {
        return __awaiter(this, void 0, void 0, function* () {
            let dbFeats = yield this.classFeatures.where('classKind')
                .equals(cls)
                .and(f => f.level <= level && f.optionId == optionId)
                .toArray();
            let ret = new Array(dbFeats.length);
            for (let i = 0; i < dbFeats.length; i++) {
                ...
                let dbOpts = yield this.getOptionsForClassFeature(dbFeats[i].id);
                if (dbOpts && dbOpts.length > 0) {
                    let options = new Array(dbOpts.length);
                    for (let j = 0; j < dbOpts.length; j++) {
                        let dbOpt = dbOpts[j];
                        let feats = yield this.getClassFeatures(cls, level, dbOpt.id);
    ...
    getSingleLevelFeatures(cls, level, optionId = undefined) {
        return __awaiter(this, void 0, void 0, function* () {
            let dbFeats = yield this.classFeatures.where('classKind')
                .equals(cls)
                .and(f => f.level === level && f.optionId == optionId)
    ...
    getOptionsForClassFeature(featId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.classFeatureOptions.where('featId').equals(featId).toArray();
        });
    }
    getFeaturesForOption(optId) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.classFeatures.where('optionId').equals(optId).toArray();
        });
    }
}
...
function seed(db) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let classInfo = yield fetch(window.location.href + 'classes.json')
            .then(res => res.json());
        console.info('seeding class info');
        yield seedClassInfo(db, classInfo);
        console.info('seeding characters');
        let chs = yield seedCharacters(db);
        yield db.characters.bulkPut(chs);
        console.info('seeding seeds');
        yield db.seeds.put({ when: new Date().toISOString(), version: db.verno });
    });
}
exports.seed = seed;
function seedClassInfo(db, info) {
    return __awaiter(this, void 0, void 0, function* () {
        for (let cls in info) {
            for (let feat of info[cls].features) {
                yield insertFeature(db, cls, feat);
            }
        }
    });
}
function insertFeature(db, cls, feat, optId) {
    return __awaiter(this, void 0, void 0, function* () {
        let dbFeat = {
            classKind: cls,
            name: feat.name,
        ...
        let featId = yield db.classFeatures.put(dbFeat);
        if (feat.options) {
            for (let opt of feat.options) {
                let dbOpt = {
                    featId,
                ...
                let optId = yield db.classFeatureOptions.put(dbOpt);
                if (opt.features) {
                    for (let subFeat of opt.features) {
                        yield insertFeature(db, cls, subFeat, optId);
                    }
                }
            }
        }
...
function seedCharacters(db) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let details = yield db.getClassDetails(class_1.ClassKind.Rogue, 4);
        let d = new character_1.Character('Daggers', new abilityScore_1.AbilityScores([
            ...
            new abilityScore_1.AbilityScore(15, abilityScore_1.AbilityKind.Dexterity),
            ...
            new abilityScore_1.AbilityScore(15, abilityScore_1.AbilityKind.Wisdom),
            new abilityScore_1.AbilityScore(15, abilityScore_1.AbilityKind.Charisma),
        ]), new race_1.Race(race_1.RaceKind.Human), new class_1.Class(class_1.ClassKind.Rogue, 4, details, class_1.DEFAULT_BONUS_ABILITY_SCORES, [
            skills_1.SkillKind.Acrobatics,
            skills_1.SkillKind.Perception,
            skills_1.SkillKind.Persuasion,
            skills_1.SkillKind.SleightOfHand,
        ]), background_1.Background.Criminal(), character_1.Alignment.TrueNeutral(), 5116, new character_1.Height(5, 8), 160, 'Blue', 0, new character_1.Armor(character_1.LightArmor.Leather, character_1.ArmorWeight.Light, 2), null, new skills_1.Skills(), [new character_1.Weapon('Dagger', character_1.WeaponType.Melee, character_1.WeaponKind.Simple, character_1.WeaponDamageKind.Piercing, character_1.WeaponWeight.Light, character_1.WeaponHandedness.One, [1, 4], new range_1.Range(5), 1, new range_1.Range(20, 60), false, true),
        ...
        ], new character_1.Wealth(), [character_1.NormalLanguage.Common], 0, 0, [], []);
        return [d];
    });
}
